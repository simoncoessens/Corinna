...
import { Scale, AlertCircle, RotateCcw, Globe, ChevronDown, ExternalLink, Building2, MapPin, PenLine, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <div className="flex flex-col items-center text-center mb-8">
        <div className="w-14 h-14 bg-[#f5f5f4] border border-[#e7e5e4] flex items-center justify-center mb-5">
          <PenLine className="w-6 h-6 text-[#57534e]" strokeWidth={1.5} />
        </div>
        <h2 className="font-serif text-2xl text-[#0a0a0a] mb-2">
          Company Details
        </h2>
        <p className="font-sans text-sm text-[#78716c] max-w-xs">
          Enter the organization you&apos;d like to assess for DSA compliance.
        </p>
      </div>

      <div className="space-y-4">
        <div>
          <label className="block font-mono text-[10px] text-[#78716c] uppercase tracking-wider mb-2">
            Organization name
          </label>
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && canSubmit) {
                  onSubmit(name.trim(), country.trim());
                }
              }}
        ...
        <div>
          <label className="block font-mono text-[10px] text-[#78716c] uppercase tracking-wider mb-2">
            ...
            <span className="text-[#a8a29e] ml-1 normal-case tracking-normal">(optional)</span>
          </label>
          <div className="relative group">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
      ...
      <div className="flex gap-3 mt-8">
        ...
        <Button
          onClick={() => onSubmit(name.trim(), country.trim())}
          disabled={!canSubmit}
          variant="primary"
          size="lg"
...
export default function AssessmentPage() {
    ...
    if (isManualEntry) {
      switch (researchStep) {
        case "manual_company":
          ...
          break;
        case "manual_scope":
          setResearchStep("manual_company");
          break;
        case "manual_size":
  ...
  const handleManualCompanySubmit = (name: string, country: string) => {
    const manualCompany: CompanyMatch = {
      name,
      top_domain: "",
      confidence: "low",
      summary_short: `Manual entry for ${name} in ${
              ...
              {currentPhase === "research" && (
                <>
                  ...
                  {researchStep === "manual_company" && (
                    ...
                    >
                      <ManualCompanyForm
                        onSubmit={handleManualCompanySubmit}
                        onBack={handleBack}
                      />
                    </motion.div>
                  )}
